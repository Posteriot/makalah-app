...
export default defineSchema({
  ...
  naskahSnapshots: defineTable({
    ...
    // Title resolution outcome (mirrors NaskahCompiledSnapshot in
    // src/lib/naskah/types.ts).
    title: v.string(),
    titleSource: v.union(
    ...
    // Compiled academic sections in canonical order. Explicit object shape
    // (NOT v.any()) so a compiler bug that produces a malformed section
    // explodes at write time instead of being silently persisted.
    sections: v.array(
      v.object({
        key: v.union(
          v.literal("abstrak"),
    ...
    // Provenance refs per stage that the compiler considered (gated by
    // validatedAt). usedForRender is true only when the ref supplied bytes
    // to a rendered section. resolution captures the resolver outcome.
    ...
    // Materialized availability — stored, not re-derived on read.
  ...
  // ════════════════════════════════════════════════════════════════
  // Naskah Views - per-user last-viewed-revision tracking
  // Used to derive `update pending` (latestRevision != lastViewedRevision).
  // Logical single-row contract per (sessionId, userId).
  // ════════════════════════════════════════════════════════════════
  ...
  pageContent: defineTable({
    ...
    headingImageDarkId: v.optional(v.id("_storage")),
    headingImageLightId: v.optional(v.id("_storage")),
