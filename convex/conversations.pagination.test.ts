...
import { listConversationsPaginated } from "./conversations"
...
describe("conversation pagination query", () => {
  beforeEach(() => {
    vi.clearAllMocks()
    vi.mocked(verifyAuthUserId).mockResolvedValue({ _id: "user_1" } as never)
  })

  it("mengembalikan slice halaman yang benar beserta total count", async () => {
    ...
    const pageOne = await callQuery<
      { userId: string; page: number; pageSize: number },
      { items: Array<{ title: string }>; totalCount: number; page: number; pageSize: number }
    >(listConversationsPaginated as never, db, {
      userId: "user_1" as never,
      page: 1,
      pageSize: 10,
    })
    const pageThree = await callQuery<
      { userId: string; page: number; pageSize: number },
      { items: Array<{ title: string }>; totalCount: number; page: number; pageSize: number }
    >(listConversationsPaginated as never, db, {
      userId: "user_1" as never,
    ...
    expect(pageOne.totalCount).toBe(23)
    expect(pageOne.items).toHaveLength(10)
    expect(pageOne.items[0]?.title).toBe("Chat 23")
    expect(pageOne.items[9]?.title).toBe("Chat 14")
    expect(pageThree.totalCount).toBe(23)
    expect(pageThree.items).toHaveLength(3)
    expect(pageThree.items[0]?.title).toBe("Chat 3")
    expect(pageThree.items[2]?.title).toBe("Chat 1")
  })
})
