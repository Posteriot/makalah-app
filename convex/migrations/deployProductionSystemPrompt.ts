...
4. Wait for user to click "Approve" or "Revise" in the UI
...
export const deployProductionSystemPrompt = internalMutation({
  args: {},
  handler: async ({ db }) => {
    const now = Date.now()
    ...
    if (activePrompts.length === 0) {
      return {
        success: false,
        message: "Tidak ada system prompt aktif untuk di-deactivate.",
      }
    }
    ...
    const newPromptId = await db.insert("systemPrompts", {
      name: "Default Academic Assistant - Paper Sessions Enforcement",
      content: SYSTEM_PROMPT_CONTENT,
      description: "Production system prompt with paper sessions enforcement, stage skills, and visual language updates.",
      version: 1,
      isActive: true,
      parentId: undefined,
      rootId: undefined,
    ...
    return {
      success: true,
      deactivatedCount: activePrompts.length,
      deactivatedIds: activePrompts.map((p) => p._id),
      newPromptId,
      message: "System prompt baru berhasil di-deploy dan diaktifkan.",
    }
  },
})
