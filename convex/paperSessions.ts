...
export const compileDaftarPustaka = mutation({
    args: {
        sessionId: v.id("paperSessions"),
        mode: v.optional(v.union(v.literal("preview"), v.literal("persist"))),
        includeWebSearchReferences: v.optional(v.boolean()),
    },
    handler: async (ctx, args) => {
        const { session } = await requirePaperSessionOwner(ctx, args.sessionId);
        const mode = args.mode ?? "persist";
        ...
        console.log(
            ...
            `mode=${mode} stage=${session.currentStage} ` +
        ...
        return {
            success: true,
            mode,
            stage: mode === "persist" ? ("daftar_pustaka" as const) : session.currentStage,
