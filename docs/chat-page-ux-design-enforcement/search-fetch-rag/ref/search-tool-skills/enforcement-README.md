...
| 6th | `CHOICE_YAML_SYSTEM_PROMPT` (optional) | ~2K | Injected when `isDraftingStage=true` — instructs model to emit choice cards via YAML fenced blocks for json-renderer |
...
data-spec { type: "patch"|"flat", ... }    → YAML spec patches (when isDraftingStage, for choice card capture)
...
### YAML Rendering & Choice Card Capture (conditional)

When `isDraftingStage=true` (paper workflow drafting stages), the compose stream is piped through `pipeYamlRender()` from `@json-render/yaml`. This transform intercepts YAML fenced blocks in the model output and emits `data-spec` stream events containing JSON patches. The orchestrator captures these patches via `applySpecPatch()` to reconstruct a `capturedChoiceSpec`, which is passed to `onFinish()` for database persistence. The UI renders these as interactive choice cards via `@json-render/react`.
...
The citation normalizer (`citations/normalizer.ts`) exports two format-specific functions:

| Function | Used By | Input Format |
|----------|---------|--------------|
| `normalizeSourcesList()` | Perplexity, Grok | Array of `{ url, title? }` from `result.sources` — unified handler for all retrievers that return a flat sources array |
| `normalizeGoogleGrounding()` | Google Grounding | Nested `providerMetadata.google.groundingMetadata` with chunks + supports |

Helper: `filterBlockedDomains()` — universal post-filter applied by each retriever's `extractSources()` method.
...
| `src/lib/ai/web-search/orchestrator.ts` | Three-phase search flow: Phase 1 retrieve, Phase 1.5 FetchWeb, Phase 2 compose + stream + RAG ingest |
