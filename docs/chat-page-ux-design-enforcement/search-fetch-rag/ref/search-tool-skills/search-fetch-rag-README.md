...
+ CHOICE_YAML_SYSTEM_PROMPT (when isDraftingStage — paper drafting)
    ↓
Compose model (Gemini 2.5 Flash) → streaming response
    ↓ (when isDraftingStage)
pipeYamlRender() → intercepts YAML fenced blocks → data-spec events
    ↓
capturedChoiceSpec → persisted in onFinish for choice card UI
```
...
When `isDraftingStage=true` (paper workflow drafting stages), the compose stream is piped through `pipeYamlRender()` which intercepts YAML fenced blocks and emits `data-spec` stream events. The orchestrator captures these via `applySpecPatch()` to reconstruct a `capturedChoiceSpec` for database persistence. Reasoning chunks are also forwarded as `data-reasoning-thought` events when transparent reasoning is enabled.
