...
- Metadata extraction: `article.byline`, `article.publishedTime`, `article.siteName` are read from readability output but NOT persisted in `FetchedContent`. Only `title` is forwarded to RAG ingest via `metadata.title`. Author, date, and site name are currently discarded.
