...
| Active stage skill mirror export | `convex/stageSkills.ts`, `convex/schema.ts`, `.references/system-prompt-skills-active/updated-3/*.md` | DB-managed stage skill snapshot | admin-managed | `export mirror` | `src/agent/managed/stage-skills/<stage-scope>/` | Stage skill layak diekspor ke managed mirror untuk diff dan sync eksplisit. Namun file mirror harus mempertahankan model catalog + version statuses `draft/published/active/archived`, bukan menjadi source runtime baru. |
