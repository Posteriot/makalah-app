...
**Scope:** Add pause/resume semantics backed by persisted run status + a new `harnessDecisions` table matching the research doc `DecisionState` contract. Bridge toward durable orchestration without building worker farms.

**Goal:** Establish the infrastructure for paused runs and user-driven resumption. The `requiresApproval` trigger stays latent (Q1=C) — no enforcer currently sets it, Phase 8 builds the infra only. Phase 9+ can add enforcer conditions later.

**Runtime namespace:** `src/lib/chat-harness/runtime/` (extends Phase 7), `src/lib/chat-harness/persistence/` (extends Phase 6), `convex/` (new `harnessDecisions.ts` + mutations on `harnessRuns.ts`)

**Explicit NON-goals (same as plan):** NO subagents, NO multi-lane durable worker farm, NO paper workflow redesign. `paperSessions` remains authoritative for domain workflow.

> **Pre-verify patch notes (2026-04-16):** Reviewer found 5 BLOCKERS in the original ~50-line spec. User decisions on design points (Q1-Q6):
>
> - **Q1 (requiresApproval trigger):** C — leave latent. Phase 8 implements infrastructure only. No enforcer sets `requiresApproval = true` in Phase 8. Step 8.5 seam exists; when a future enforcer sets it, the path works end-to-end.
> - **Q2 (Decision table):** A — new `harnessDecisions` Convex table matching research doc `DecisionState` contract (lines 838-862). `paperSessions` approval is domain truth; harness decisions are runtime facts. Separate tables, no cross-contamination.
> - **Q3 (Resume mechanics):** A — NEW POST `/api/chat` with `x-harness-resume: <runId>` header. Client approves → calls `resumeRun` mutation → POSTs `/api/chat` with resume header. Orchestrator detects resume, loads persisted lane, skips step 2 only (resolveRunLane), re-runs other steps normally. No background worker.
> - **Q4 (UI wiring):** ADDITION — `resumeRun` runs BEFORE `approveStage`, not as replacement. Keeps domain workflow (approveStage) orthogonal to runtime resumption. Only fires when `harnessRun.status === "paused"`.
> - **Q5 (Event emission):** YES — emit `run_paused` + `run_resumed`. Both in canonical 29 event types from Phase 6 registry; Phase 8 adds emitters.
> - **Q6 (Step lifecycle):** No active step during pause. Phase 7 pause check (step 8.5) fires BEFORE `buildStepStream`, so no step is ever started at pause time. No partial step state to manage.

**Reference:**
- Research doc `DecisionState` contract: `.references/agent-harness/research/2026-04-15-makalahapp-harness-v1-event-model-and-data-contracts.md` lines 838-862
- Phase 7 pause seam: `src/lib/chat-harness/runtime/orchestrate-sync-run.ts:223-230`
- Phase 6 persistence contracts: `src/lib/chat-harness/persistence/types.ts`

---

### Task 8.1: Schema + Convex mutations for pause/resume

#### 8.1a: `harnessDecisions` table (new)

**Modify:** `convex/schema.ts`

New table matching research doc `DecisionState`:
...
**Important:** `harnessRuns.pendingDecisionId` stays `v.optional(v.string())`. It carries the `decisionId` string when set. Not a `v.id("harnessDecisions")` because decisionId is a stable semantic identifier (client-facing, correlation), while Convex `_id` is internal.

#### 8.1b: `convex/harnessDecisions.ts` (new file)

Mutations + queries:

- `createDecision({ runId, type, blocking, workflowStage, prompt, decisionId? })` → `{ decisionId }` — generates UUID if omitted, inserts with `status="pending"`, `requestedAt=Date.now()`
- `resolveDecision({ decisionId, response, resolution })` → `null` — patches status=`"resolved"` or `"declined"`, sets `response`, `resolvedAt`
- `invalidateDecision({ decisionId })` → `null` — patches status=`"invalidated"` (used if pause is canceled administratively)
- `getDecision({ decisionId })` → record or null (via `by_decisionId` index + `.unique()`)
- `getPendingByRun({ runId })` → latest pending decision for a run, or null
- `getDecisionsByRun({ runId, limit? })` → all decisions for a run, ordered by `requestedAt` desc

Auth: `requireConversationOwner(ctx, run.conversationId)` via the run's conversation. Fetching the run first to resolve conversationId.

#### 8.1c: `pauseRun` + `resumeRun` on `convex/harnessRuns.ts`

Add two mutations:

- `pauseRun({ runId, reason, decisionId })` — patches `status="paused"`, `pausedAt=Date.now()`, `pendingDecisionId=decisionId`, `updatedAt=Date.now()`. Verifies `ownerToken` match not required here (same conversation auth as other harnessRuns mutations). Input `reason` captured in `policyState.lastPolicyReason` (not a new field).
- `resumeRun({ runId, ownerToken })` — validates the run's `ownerToken` matches (strict match — this mutation is the sole resume entrypoint, so it double-checks ownership). Patches `status="running"`, `pendingDecisionId=undefined`, clears `pausedAt`, sets `updatedAt=Date.now()`. If run is not currently `paused`, throws `"run is not paused"`.

**Acceptance criteria for Task 8.1:**
- Schema compiles
- `npx convex dev` accepts new table + mutations
- All mutations have access control via ownership checks
- `harnessDecisions.ts` ≤200 lines
- Zero changes to domain tables

**Test:** `npx tsc --noEmit` + new test file `convex/harnessDecisions.test.ts` covering createDecision/resolveDecision/getPendingByRun + `pauseRun`/`resumeRun` lifecycle in `convex/harnessRuns.test.ts`

---

### Task 8.2: Persistence adapter pause/resume + event emitters

#### 8.2a: RunStore interface extension

**Modify:** `src/lib/chat-harness/persistence/types.ts`
...
export interface RunStore {
    // ... existing methods ...
    pauseRun(runId: Id<"harnessRuns">, params: PauseRunParams): Promise<{ decisionId: string }>
    resumeRun(runId: Id<"harnessRuns">, params: ResumeRunParams): Promise<void>
}
```

`DecisionPrompt` mirrors the schema `prompt` object shape; defined in the same types file.

#### 8.2b: run-store.ts implementation

**Modify:** `src/lib/chat-harness/persistence/run-store.ts`

`pauseRun` composes 2 mutations in order:
1. `createDecision(...)` → `{ decisionId }`
2. `pauseRun(runId, reason, decisionId)` — Convex mutation sets the paused state + pendingDecisionId

`resumeRun` composes 2 mutations:
1. `resolveDecision(decisionId, response, resolution)`
2. `resumeRun(runId, ownerToken)` — Convex mutation validates + flips state

Both log `[HARNESS][persistence]` lines after success (matching existing pattern).

#### 8.2c: EventStore emit helpers (optional typed wrappers)

Canonical event types `run_paused` and `run_resumed` exist in `event-types.ts` constant. Phase 8 emits them directly via `eventStore.emit({ eventType: HARNESS_EVENT_TYPES.RUN_PAUSED, ... })` — no new adapter methods needed.

Payload shapes (inline, not new types):
- `run_paused`: `{ reason, decisionId, workflowStage, pausedAt }`
- `run_resumed`: `{ decisionId, resolution, response?, resumedAt }`

**Acceptance criteria for Task 8.2:**
- `npx tsc --noEmit` clean
- New adapter tests covering `pauseRun`/`resumeRun` composition (~4 tests)
- Observability logs fire after successful mutations

**Test:** `npx tsc --noEmit` + extend `src/lib/chat-harness/persistence/run-store.test.ts`

---

### Task 8.3: Orchestrator resume detection + step 8.5 wiring

#### 8.3a: Resume context in entry layer

**Modify:** `src/lib/chat-harness/types/runtime.ts`

Extend `AcceptedChatRequest` with optional resume context:

```ts
export interface AcceptedChatRequest {
    // ... existing fields ...
    /** Present when the incoming request carries `x-harness-resume: <runId>` header. */
    resumeContext?: {
        runId: Id<"harnessRuns">
        ownerToken: string   // extracted from persisted run for validation
    }
}
```

**Modify:** `src/lib/chat-harness/entry/accept-chat-request.ts`

- Parse `x-harness-resume` request header
- If present: fetch run via `runStore.getRunByOwnerToken` (or new `getRunById` if needed), verify caller owns the conversation, populate `resumeContext`
- Fail fast with `Response(..., { status: 403 })` if token/ownership check fails
- If header absent or run not in `paused` status: clear `resumeContext` (treat as fresh request)

#### 8.3b: Orchestrator resume path

**Modify:** `src/lib/chat-harness/runtime/orchestrate-sync-run.ts`

At top of `orchestrateSyncRun`:

```ts
const isResume = !!ctx.accepted.resumeContext
```

Step 2 (resolveRunLane) behavior:
- If `isResume`: load existing lane from persisted run (reuse runId, ownerToken, sessionId). Emit `run_resumed` event. No new harnessRuns row created.
- If NOT resume: current behavior (create new run, emit `run_started`).

Step 5 (resolvePaperContext):
- Re-runs normally. paperSessionId may already be linked to the run; paper state is fetched fresh.

Step 8.5 (pause check):
- When policyDecision.requiresApproval is true: call `runStore.pauseRun(...)` with reason + decision prompt, emit `run_paused`, return `{ kind: "paused", runId, pendingDecisionId: decisionId }`.
- On RESUME path: the prior decision was resolved via `resumeRun` BEFORE this POST, so the policy re-evaluation should see an approved state. Orchestrator doesn't need to track this explicitly — if `requiresApproval` is still true, it pauses again (which is correct behavior if user approval wasn't actually provided).

Steps 9-13: run normally. No resume-specific divergence.

#### 8.3c: `run-chat-harness.ts` update

**Modify:** `src/lib/chat-harness/runtime/run-chat-harness.ts`

The 202 paused response now includes a populated `pendingDecisionId` (not null placeholder):
...
if (result.kind === "paused") {
    return new Response(
        JSON.stringify({
            status: "paused",
            runId: result.runId,
            pendingDecisionId: result.pendingDecisionId,  // real decision ID now
        }),
...
**Acceptance criteria for Task 8.3:**
- `x-harness-resume` header handled in `acceptChatRequest`
- Resume skips resolveRunLane correctly, emits `run_resumed`
- Pause path creates real decision row + returns decisionId
- `run_paused` + `run_resumed` events fire with correct payloads
- All existing tests still pass (no behavior change for non-resume, non-paused requests)

**Test:** `npx tsc --noEmit` + targeted test for resume context in `acceptChatRequest` + baseline 189/189

---

### Task 8.4: UI handler for 202 pause response + resumeRun wiring

**Modify:** `src/components/chat/ChatWindow.tsx` (approve/revise handlers)
**Modify:** `src/lib/hooks/usePaperSession.ts` OR new hook `useHarnessRun.ts`
**Modify:** POST logic in chat client to read 202 responses

#### 8.4a: 202 response detection

Client-side POST `/api/chat` wrapper:
- Check response status. If 202, parse JSON body `{ status, runId, pendingDecisionId }`, store in local state (e.g., via new `harnessRunState` atom or React context).
- Display existing `PaperValidationPanel` "awaiting approval" state — same UI, new trigger condition.

#### 8.4b: Approve flow (ADDITION per Q4)

Existing `handleApprove` in `ChatWindow.tsx:2161-2189`:

```ts
// BEFORE
await approveStage(userId)
sendMessageWithPendingIndicator("...")
```

After Phase 8:

```ts
// If there's a paused harness run, resume it FIRST (runtime layer)
const pausedRun = await fetchQuery(api.harnessRuns.getRunByConversation, {
    conversationId, statusFilter: "paused"
})
if (pausedRun && pausedRun.pendingDecisionId) {
    await resumeRun({
        runId: pausedRun._id,
        ownerToken: pausedRun.ownerToken,
        decisionResponse: {
            decisionId: pausedRun.pendingDecisionId,
            resolution: "resolved",
...
// Then proceed with domain workflow transition
await approveStage(userId)

// Re-POST chat with resume header so orchestrator continues
sendMessageWithPendingIndicator("...", { harnessResumeRunId: pausedRun?._id })
```

The `sendMessageWithPendingIndicator` helper adds `x-harness-resume: <runId>` header when invoked with resume context.

#### 8.4c: Revise flow (mirror)

`handleRevise` extends identically: `resumeRun(..., resolution: "resolved", response: { decision: "revise", feedback })` THEN `requestRevision(...)`.

**Acceptance criteria for Task 8.4:**
- Client detects 202 pause response and stores pending state
- Approve/revise call `resumeRun` before domain mutation when paused run exists
- Next chat POST includes `x-harness-resume` header
- No regression in existing non-paused flow

**Test:** `npx tsc --noEmit` + baseline 189/189 + manual UI smoke

---

### Task 8.5: Post-phase review checkpoint

**Action:** Dispatch agent reviewer to audit Phase 8 against:

1. **Schema fidelity:** `harnessDecisions` matches research doc `DecisionState` contract
2. **Decision table separation:** `harnessDecisions` is independent of `paperSessions`; no foreign keys to paper tables
3. **Mutation correctness:** `pauseRun`/`resumeRun` enforce ownership via `ownerToken`, validate status transitions, are idempotent where safe
4. **Adapter thinness:** `run-store.pauseRun` / `.resumeRun` compose Convex mutations without business logic
5. **Orchestrator resume path:** only step 2 (resolveRunLane) is skipped; all other steps run normally
6. **Event emission:** `run_paused` + `run_resumed` fire with correct correlationId + payloads
7. **UI wiring ADDITION (not replacement):** `approveStage`/`requestRevision` are still called; `resumeRun` runs BEFORE them when paused run exists
8. **Step lifecycle:** no partial step state — pause only at step 8.5 which runs before step 11
9. **`paperSessions` untouched:** domain workflow unchanged; all approveStage/requestRevision semantics preserved
10. **Observability:** new `[HARNESS][persistence] pauseRun` / `[HARNESS][event] run_paused` / `[HARNESS][event] run_resumed` logs fire

**Manual smoke required:**
- Trigger pause via test enforcer (add a temporary test-only enforcer that sets `requiresApproval=true` for verification, then remove — or manually set it in dev)
- Verify 202 response + decision row + event row
- Approve in UI → verify resumeRun runs + `x-harness-resume` header on next POST + orchestrator continues from step 9

**Output:** Review report → fix gaps → report to user → FINAL validation before merge.
