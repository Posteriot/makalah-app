...
Progress branch ini saat ini paling maju di area `global header`.
...
- test khusus header, dropdown, dan hook user state sudah tersedia
...
1. `context.md`
2. `progress-log.md`
3. paket dokumen `global-header/` bila butuh memahami pola kerja yang sudah terjadi
...
- fokus berikutnya yang paling masuk akal: audit dan migrasi halaman marketing lain dengan baseline `home`, `chat`, dan `global header`
