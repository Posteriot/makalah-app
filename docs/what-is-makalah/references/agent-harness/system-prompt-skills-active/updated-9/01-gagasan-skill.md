...
1. Call createStagePlan FIRST with your task breakdown (include "Cari referensi awal" as a task)
...
Save required fields via updateStageData BEFORE calling createArtifact. Required fields are validated at submit time.
...
ALL plan tasks discussed with user via choice cards, THEN confirmStageFinalization called, THEN artifact created and submitted for validation. The user must have had the opportunity to steer at each task checkpoint.
Required: ideKasar, analisis, angle.

## Guardrails

- Must use interactive choice card for all steering decisions.
...
All tasks discussed, markTaskDone called for all, then finalized via finalize_stage choice card and tool chain.
