...
export default function SignUpPage() {
  ...
  const markAsRegistered = useMutation(api.waitlist.markAsRegistered)

  // Pre-fill form fields from invite token data
  const isInviteMode = !!inviteToken && tokenValidation?.valid === true
  const prefilled = useRef(false)

  useEffect(() => {
    if (isInviteMode && !prefilled.current && tokenValidation?.valid) {
      prefilled.current = true
      if (tokenValidation.email) setEmail(tokenValidation.email)
      if (tokenValidation.firstName) setFirstName(tokenValidation.firstName)
      if (tokenValidation.lastName) setLastName(tokenValidation.lastName)
    }
  }, [isInviteMode, tokenValidation])
  ...
  async function handleEmailSignUp(e: React.FormEvent) {
    ...
    try {
      ...
      if (result.error) {
        ...
        // Mark waitlist entry as registered if this is an invite sign-up
        if (isInviteMode) {
          try {
            await markAsRegistered({ email: email.trim() })
          } catch {
            // Non-critical — don't block sign-up flow
            console.warn("[Waitlist] Failed to mark as registered")
          }
        }
  ...
  function renderSignUpForm() {
    return (
      <div className="w-full space-y-5">
        {/* Google OAuth — hidden for invite users */}
        {!isInviteMode && (
          <>
            <button
              type="button"
