 ...
 *
 * Export completed paper session ke format PDF.
 *
 * POST /api/export/pdf
 * Body: { sessionId: string }
 *
 * Response:
 * - 200: PDF file stream dengan Content-Disposition header
...
import { generatePDFStream, getPDFFilename } from "@/lib/export/pdf-builder"
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 7. Get filename
    const filename = getPDFFilename(content.title)
    ...
    console.error("[PDF Export] Server error:", error)
