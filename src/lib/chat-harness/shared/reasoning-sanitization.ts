...
export const MAX_REASONING_TRACE_STEPS = 8
export const MAX_REASONING_TEXT_LENGTH = 240

export const ALLOWED_REASONING_STEP_KEYS = new Set([
    "intent-analysis",
    "paper-context-check",
    "search-decision",
    "source-validation",
...
export const ALLOWED_REASONING_STATUSES = new Set([
    "pending",
    "running",
    "done",
    "skipped",
...
export const truncateReasoningText = (value: string) => value.slice(0, MAX_REASONING_TEXT_LENGTH)
export const collapseSpaces = (value: string) => value.replace(/\s+/g, " ").trim()

export const containsForbiddenReasoningText = (value: string) =>
    FORBIDDEN_REASONING_PATTERNS.some((pattern) => pattern.test(value))

export const sanitizeReasoningText = (value: string, fallback: string) => {
...
export const normalizeLegacyReasoningNote = (note: string) => {
...
export const sanitizeReasoningMode = (mode: unknown): "normal" | "paper" | "websearch" | undefined => {
    if (mode === "normal" || mode === "paper" || mode === "websearch") return mode
    return undefined
}

export const sanitizeReasoningStatus = (status: unknown): "pending" | "running" | "done" | "skipped" | "error" => {
...
export function sanitizeReasoningTraceForPersistence(
